...
def main(argv):
    ...
    try:
      ...
      print('handball_rapm_7v7.py -i <inputfile> -o <outputfile>')
      sys.exit(2)
    for opt, arg in opts:
      if opt == '-h':
         print('handball_rapm_7v7.py -i <inputfile> -o <outputfile>')
    ...
    players = list(set(set(df.P1.unique()) | set(df.P2.unique())| set(df.P3.unique())| set(df.P4.unique()) | set(df.P5.unique()) | set(df.P6.unique()) | set(df.P7.unique())))
    ...
    def obj(x):
        ...
        home_pred = df[val_cols[0:7]].apply(lambda i: x[i-1]).sum(axis=1)
        pred_diff = home_pred
